 ...
 */
    ...
    if (removeLeft > 0) {
      ...
      self.cleanLeft = leftTestIndex;
    }

    if (removeRight > 0) {
      self._removeItems(rightTestIndex, removeRight);
      self.cleanRight = rightTestIndex;
    }
  },
   ...
   * @param {boolean} toAdd
   */
  _removeItems: function(start, amount, toAdd) {
    var self = this,
      item, index;
    ...
    while (amount) {
      ...
      if (item) {
        toAdd ? self._plantItem(index) : item.dispose();
      }
