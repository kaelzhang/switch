 ...
 */
Switch.prototype._lifeCycle = ['_before', '_on', '_after'];
...
Switch.prototype.prev = function(circular){
    ...
    (circular || !self.noprev) && self.switchTo( circular ? expect % self.length : expect );
...
Switch.prototype.next = function(circular){
    ...
    self.emit(NEXT);
    (circular || !self.nonext) && self.switchTo( circular ? expect % self.length : expect );
 ...
 * @onceBefore _runLC
 */
Switch.prototype._initLC = function(){
...
Switch.prototype._dealNav = function(type){
    ...
    if(nav){
        ...
        self.emit((isEnd ? EVENT_NAV_DISABLE : EVENT_NAV_ENABLE), [nav, type]);
    }
};
